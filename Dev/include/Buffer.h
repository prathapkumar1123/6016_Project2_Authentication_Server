...
public:
    ...
    void EnsureCapacity(size_t size)
    {
        ...
        {
            //m_Size = std::max(m_Size * 2, m_WriteIndex + size);
            size_t newSizeCandidate = m_Size * 2;
            if (newSizeCandidate < m_WriteIndex + size) {
                newSizeCandidate = m_WriteIndex + size;
            }

            m_Size = newSizeCandidate;
